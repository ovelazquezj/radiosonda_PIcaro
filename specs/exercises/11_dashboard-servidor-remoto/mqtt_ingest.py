...
A diferencia del ej.10, aqui el broker es REMOTO y exige TLS y usuario/contrasena
(puerto 8883). El del ej.00 era anonimo y en claro (localhost:1883): si vuelves a
uno asi, pon 'tls' en false y omite las credenciales — el codigo soporta ambos.
"""
import json
import socket
import ssl
...
def resolve_host(host):
    """host='auto' (o vacio) -> IP LAN del equipo.

    Resto: se usa tal cual. El modo 'auto' solo tenia sentido con el broker
    local en Docker, donde otro broker en 127.0.0.1:1883 podia acaparar
    'localhost'. Contra un host remoto no se toca nada."""
...
class MqttIngest:
    ...
    def __init__(self, host, port, app_id, dev_eui, on_record, on_status,
                 username=None, password=None, tls=False, ca_certs=None):
        ...
        if tls:
            # ca_certs=None -> almacen de CAs del sistema, que ya incluye a
            # Let's Encrypt. Solo hace falta indicar un fichero si algun dia el
            # broker usara una CA propia.
            self.client.tls_set(ca_certs=ca_certs, cert_reqs=ssl.CERT_REQUIRED)
