#!/usr/bin/env bash
# Adjunta payload_decoder.js al device profile del TTGO para que ChirpStack entregue el
# campo 'object' ya decodificado en el MQTT (la base del dashboard).
#   export TOKEN="..."; ./scripts/upload_codec.sh <DEVICE_PROFILE_ID>
set -euo pipefail
: "${TOKEN:?Exporta TOKEN}"
API="${API:-http://localhost:8090}"
DP="${1:?Pasa el device-profile-id (p.ej. 150ae510-0577-4352-9cd3-39c354889474)}"
...
print(f"Codec JS subido al device profile {dp}. Los próximos uplinks traerán 'object' decodificado.")
PY
